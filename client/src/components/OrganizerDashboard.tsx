...
const OrganizerDashboard = () => {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('receive_telemetry', (data) => {
      setTelemetryLogs(logs => [...logs.slice(-20), `GPS: ${data.trekName} [${data.lat.toFixed(4)}°, ${data.lng.toFixed(4)}°] Alt: ${data.alt}m`]);
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const filteredTreks = (Object.values(allTreks) as any[]).filter(
        (t) => t.organizer && t.organizer.toLowerCase() === user.name.toLowerCase()
      );
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      console.error('Error fetching notifications in Organizer Dashboard:', err);
  ...
  const handleCreateTrekSubmit = async (e) => {
    ...
    if (!title.trim() || !location.trim() || !price || !duration.trim() || !dates.trim() || !pickup.trim() || !description.trim()) {
      ...
      if (editingTrekId) {
        ...
        if (success) {
          ...
          setTimeout(() => {
            ...
            setTitle(''); setLocation(''); setPrice(''); setDuration(''); setDifficulty('Moderate');
            setDates(''); setPickup(''); setTemp('18°C Sunny'); setDescription(''); setImage('/explore_glowing_tent.png'); setImageFile(null);
        ...
        const generatedId = title.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-' + Math.floor(100 + Math.random() * 900);
        const newTrek = {
          id: generatedId,
          title: title.trim(),
          name: title.trim(),
          ...
          organizer: user.name,
          timeline: [
            { num: "01", title: "Camp Arrival & Briefing", desc: `Welcome meeting in ${location}, camp pitch, and local safety briefing.`, hasDining: true },
            { num: "02", title: "Ascent & Return", desc: "Summit ascent to witness high mountain views, packed lunch, and safe descent." }
          ],
          guide: { name: user.name, title: "Lead Organizer", treks: "1+ Treks Done", exp: "5 yrs Experience", avatar: user.name[0].toUpperCase() }
        };
        ...
        if (added) {
          setSuccessMsg('Expedition successfully published! Redirecting to dashboard...');
          setTitle(''); setLocation(''); setPrice(''); setDuration(''); setDates(''); setPickup(''); setDescription(''); setImageFile(null);
  ...
  const handleEditClick = (trek) => {
    ...
    setPrice(trek.price_num ? trek.price_num.toString() : (trek.price || '').replace(/[^0-9]/g, ''));
  ...
  const handleResubmit = async (e: React.FormEvent) => {
    ...
    if (!resubmitPhone || !resubmitAddress || !resubmitFile) {
      setResubmitError('Please fill in all verification fields and select a file.');
      return;
    }
    ...
    try {
      ...
      setResubmitError(err.response?.data?.message || 'Error occurred during resubmission. Please try again.');
    } finally {
      setIsResubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#070708] text-white flex items-center justify-center font-sans">
        <div className="flex flex-col items-center gap-4">
          <svg className="animate-spin h-8 w-8 text-trek-brown" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
          </svg>
          <span className="text-xs uppercase tracking-widest text-gray-500 font-bold">Verifying Credentials...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#070708] text-white flex items-center justify-center p-6 select-none relative overflow-hidden">
        ...
        <div className="max-w-xl w-full z-10 bg-[#0d0d0f]/90 border border-white/5 rounded-2xl p-8 backdrop-blur-md shadow-2xl relative">
          ...
          <div className="flex items-center gap-2 mb-8">
            ...
            <span className="font-outfit font-black tracking-widest text-md uppercase">TrekMate Partner</span>
          </div>

          {(orgProfile.status === 'pending' || orgProfile.status === 'not_applied') && (
            <div className="flex flex-col gap-4 text-center py-6 select-text">
              ...
              <h2 className="text-2xl font-bold uppercase tracking-tight text-white font-outfit">Verification Pending</h2>
              <p className="text-sm text-gray-400 font-light leading-relaxed">
                Your verification is under review. You will gain access once approved.
              </p>
              <div className="mt-4 bg-white/[0.02] border border-white/5 rounded-xl p-4 text-left text-xs text-gray-500 space-y-2">
                <p><strong>Organization Name:</strong> {orgProfile.organization_name || user.name}</p>
                <p><strong>Submitted On:</strong> {new Date(orgProfile.submission_date || orgProfile.createdAt || Date.now()).toLocaleDateString()}</p>
                <p><strong>Status:</strong> <span className="text-amber-500 uppercase tracking-widest font-black text-[9px] px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 ml-1">Pending Review</span></p>
              </div>
              <button
          ...
          {orgProfile.status === 'rejected' && (
            <div className="flex flex-col gap-4 select-text">
              ...
              <h2 className="text-2xl font-bold uppercase tracking-tight text-white text-center font-outfit">Verification Rejected</h2>
              
              <div className="bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl p-4 text-xs font-semibold">
                <p className="uppercase tracking-widest text-[9px] text-red-500 font-black mb-1">Rejection Reason:</p>
                <p className="font-medium text-red-300">{orgProfile.rejection_reason || 'No specific reason provided by the administrator. Please upload valid proof.'}</p>
                {orgProfile.admin_notes && (
                  <>
                    <p className="uppercase tracking-widest text-[9px] text-gray-500 font-black mt-3 mb-1">Admin Review Notes:</p>
                    <p className="font-light text-gray-400">{orgProfile.admin_notes}</p>
                  </>
                )}
              </div>
              ...
              <form onSubmit={handleResubmit} className="mt-6 flex flex-col gap-4 border-t border-white/5 pt-6 select-text max-h-[300px] overflow-y-auto pr-2">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    <option value="Company Registration Documents">Company Registration Documents</option>
                    <option value="Event Management License">Event Management License</option>
                    <option value="Any Other Supporting Document">Any Other Supporting Document</option>
                  </select>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative border border-dashed border-white/15 hover:border-trek-brown/50 transition-colors rounded-xl p-3 flex flex-col items-center justify-center gap-1 cursor-pointer bg-white/[0.01]">
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          if (file.size > 10 * 1024 * 1024) {
                            setResubmitError('File is too large. Max size is 10MB.');
                            setResubmitFile(null);
                          } else {
                            setResubmitError('');
                    ...
                    <span className="text-[10px] text-gray-400 text-center">
                      {resubmitFile ? (
                        <strong className="text-trek-brown font-black">{resubmitFile.name} ({(resubmitFile.size / (1024 * 1024)).toFixed(2)} MB)</strong>
                      ) : (
          ...
          {orgProfile.status === 'suspended' && (
            <div className="flex flex-col gap-4 text-center py-6 select-text">
              ...
              <h2 className="text-2xl font-bold uppercase tracking-tight text-white">Account Suspended</h2>
              <p className="text-sm text-gray-400 font-light leading-relaxed">
                Your publishing privileges have been suspended by the platform administrators.
              </p>
              {orgProfile.admin_notes && (
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl p-4 text-xs font-semibold text-left">
                  <p className="uppercase tracking-widest text-[9px] text-red-500 font-black mb-1">Admin Suspension Notes:</p>
                  <p className="font-light text-gray-300">{orgProfile.admin_notes}</p>
                </div>
              )}
      ...
      <header className="h-14 bg-[#0c0c0e] border-b border-white/5 flex items-center justify-between px-6 shrink-0 z-30 select-none">
        ...
        <div className="flex items-center gap-2.5">
          <Link to="/" className="flex items-center gap-2.5 text-white group">
            ...
            <span className="font-outfit font-black tracking-widest text-md uppercase bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">TrekMate</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {notifications.filter(n => !n.is_read).length > 0 && !muteNotifications && (
              <span className="absolute top-1.5 right-1.5 h-1.5 w-1.5 bg-orange-500 rounded-full" />
            )}
          </button>

          <div className="h-7 w-7 rounded-md bg-orange-600/90 hover:bg-orange-600 flex items-center justify-center font-outfit text-xs font-black text-white shadow-md transition duration-200 cursor-default" title={user.name}>
            {userInitials}
          </div>
        </div>
        ...
        <aside className="w-52 bg-[#0c0c0e] border-r border-white/5 flex flex-col justify-between shrink-0 h-full select-none z-20">
          ...
          <div className="p-3 space-y-1 overflow-y-auto flex-grow">
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'dashboard'
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'create-trip'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'manage-trips'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'bookings'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'members'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              onClick={() => setActiveView('notifications')}
              className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'notifications'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'live-tracking'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'chat'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'payments'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition duration-200 border cursor-pointer ${activeView === 'analytics'
                  ? 'bg-trek-brown/10 border-trek-brown/20 text-orange-400 font-black'
                  : 'bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/[0.01]'
                }`}
            >
          ...
          <div className="p-6 md:p-8 relative z-10 w-full max-w-7xl mx-auto flex-grow">
                  ...
                  <div className="xl:col-span-2 bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow-2xl flex flex-col justify-between min-h-[350px]">
                    ...
                    <div className="flex-1 flex flex-col justify-end mt-2">
                      ...
                      <div className="flex justify-between px-10 text-[9px] font-mono text-gray-500 font-bold select-none pt-2">
                        ...
                        <span>W3</span>
                        <span>W4</span>
                        <span className="text-orange-500 font-extrabold">W5</span>
                        <span>W6</span>
                        <span>W7</span>
                        <span>W8</span>
                      </div>
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="space-y-1.5 group">
                        ...
                        <div className="flex justify-end">
                          <span className="text-[8px] text-gray-500 font-bold font-mono">75% Complete</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 group">
                        ...
                        <div className="flex justify-between text-[8px] font-bold text-gray-500">
                          <span className="text-emerald-500 font-black">Ascent Phase</span>
                      ...
                      <div className="space-y-1.5 group">
                        ...
                        <div className="flex justify-between text-[8px] font-bold text-gray-500">
                          <span className="text-orange-400 font-black">Final Descent</span>
              ...
              <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-6 shadow-2xl max-w-4xl mx-auto animate-fadeIn">
                ...
                <form onSubmit={handleCreateTrekSubmit} className="flex flex-col gap-5 select-text">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] text-gray-500 uppercase tracking-widest font-black select-none">Trail Location</label>
                      <input
                        type="text"
                        required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] text-gray-500 uppercase tracking-widest font-black select-none">Duration</label>
                      <input
                        type="text"
                        required
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] text-gray-500 uppercase tracking-widest font-black select-none">Difficulty</label>
                      <select
                        value={difficulty}
                      ...
                      >
                        <option value="Easy" className="bg-[#0c0c0e]">Easy</option>
                        <option value="Moderate" className="bg-[#0c0c0e]">Moderate</option>
                        <option value="Difficult" className="bg-[#0c0c0e]">Difficult</option>
                        <option value="Expert" className="bg-[#0c0c0e]">Expert</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] text-gray-500 uppercase tracking-widest font-black select-none">Reporting / Pickup Point</label>
                      <input
                        type="text"
                        required
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] text-gray-500 uppercase tracking-widest font-black select-none">Weather Presets</label>
                      <input
                        type="text"
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex items-center gap-4">
                      <label className="flex-1 cursor-pointer bg-white/5 hover:bg-white/10 border border-white/10 hover:border-orange-500/50 rounded-xl px-4.5 py-4 transition flex flex-col items-center justify-center gap-2 border-dashed">
                        <svg className="w-6 h-6 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      editingTrekId ? 'Update Expedition Live' : 'Publish Expedition Live'
                    )}
                  </button>
                </form>
              </div>
            ...
            {activeView === 'manage-trips' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="flex justify-between items-center select-none">
                  <div>
                    <h2 className="font-outfit text-xl font-black uppercase text-white tracking-tight">Managed Expeditions</h2>
                    <p className="text-gray-400 text-xs font-light mt-0.5">Manage details and view schedules for your published trails.</p>
                  </div>
                  <button
                    onClick={() => setActiveView('create-trip')}
                ...
                {myTreks.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 text-center border border-white/5 rounded-2xl bg-white/[0.01]">
                    ...
                    <h3 className="font-outfit text-xl font-bold uppercase text-white mb-2">No Treks Published</h3>
                    <p className="text-gray-400 text-xs max-w-xs leading-relaxed mb-6 font-light">
                      You have not published any custom expeditions. Add your first trek to start hosting hikers.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="p-5 flex-grow flex flex-col justify-between">
                          ...
                          <div className="border-t border-white/5 pt-4 mt-5 flex items-center justify-between">
                            <div className="flex flex-col">
                              ...
                              <span className="text-sm font-black text-white mt-1">{trek.price}</span>
                            </div>
            ...
            {activeView === 'bookings' && (
              <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-6 shadow-2xl space-y-6 animate-fadeIn">
                <div className="flex justify-between items-center select-none">
                  <div>
                    <h2 className="font-outfit text-xl font-bold uppercase text-white">Registered Participants</h2>
                    <p className="text-gray-400 text-xs font-light mt-0.5">List of verified bookings and tickets for your trails.</p>
                  </div>
                </div>
                ...
                {bookings.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center select-none">
                    ...
                    <h4 className="font-outfit text-md font-bold uppercase text-gray-400 mb-1">No Bookings Yet</h4>
                    <p className="text-gray-500 text-xs max-w-xs leading-relaxed font-light">
                      When hikers register and purchase tickets for your treks, they will be listed here.
                    </p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="border-b border-white/10 text-gray-500 uppercase text-[9px] tracking-widest">
                          ...
                          <th className="py-3 px-4 font-black text-center">Seats</th>
                      ...
                      <tbody>
                        {bookings.map((booking) => (
                          <tr key={booking.ticketId} className="border-b border-white/5 hover:bg-white/[0.02] transition">
                            <td className="py-4 px-4 font-bold text-white">
                              <div>{booking.fullName}</div>
                              <div className="text-[9px] font-mono text-gray-500 mt-0.5">{booking.ticketId}</div>
                            </td>
                            <td className="py-4 px-4 font-light text-gray-400">
                              <div>{booking.email}</div>
                              <div className="font-mono text-[10px] mt-0.5">{booking.phone}</div>
                            </td>
                            ...
                            <td className="py-4 px-4 select-none">
                              {booking.booking_status === 'pending' ? (
                                <div className="flex gap-2">
                                  <button onClick={() => handleUpdateBookingStatus(booking._id, 'confirmed')} className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded cursor-pointer hover:bg-emerald-500/30 transition">
                                    Accept
                                  </button>
                                  <button onClick={() => handleUpdateBookingStatus(booking._id, 'rejected')} className="bg-red-500/10 border border-red-500/20 text-red-400 text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded cursor-pointer hover:bg-red-500/30 transition">
                                    Reject
                                  </button>
                                </div>
                              ) : (
                                <span className={`${booking.booking_status === 'confirmed' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-gray-500/10 border-gray-500/20 text-gray-400'} border text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded`}>
                                  {booking.status}
                                </span>
                              )}
                            </td>
            ...
            {activeView === 'members' && (
              <div className="space-y-6 animate-fadeIn">
                <div>
                  <h2 className="font-outfit text-xl font-black uppercase text-white tracking-tight">Personnel Directory</h2>
                  <p className="text-gray-400 text-xs font-light mt-0.5">Manage lead guides, crew, and emergency contacts.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 select-none">
                  ...
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-4.5 shadow-lg flex flex-col justify-between">
                    <div className="flex items-start gap-3.5">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-white">Sarah Williams</h4>
                        <span className="text-[8px] text-gray-500 uppercase font-black block mt-0.5">Certified Lead Guide</span>
                  ...
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-4.5 shadow-lg flex flex-col justify-between">
                    <div className="flex items-start gap-3.5">
                      ...
                      <div>
                        ...
                        <span className="text-[8px] text-gray-500 uppercase font-black block mt-0.5">Senior Trail Master</span>
                  ...
                  <div className="bg-[#0c0c0e] border border-orange-500/20 rounded-2xl p-4.5 shadow-[0_0_15px_rgba(249,115,22,0.02)] flex flex-col justify-between">
                    <div className="flex items-start gap-3.5">
                      ...
                      <div>
                        ...
                        <span className="text-[8px] text-orange-400 uppercase font-black block mt-0.5">HQ Operations Command</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {Object.keys(tripParticipants).length === 0 ? (
                      <div className="col-span-full flex flex-col items-center justify-center py-10 text-center border border-white/5 rounded-2xl bg-white/[0.01]">
                        <h4 className="font-outfit text-md font-bold uppercase text-gray-400 mb-1">No Participants Yet</h4>
                        <p className="text-gray-500 text-xs max-w-xs leading-relaxed font-light">
                          Once members join your trips, they will be listed here.
                        </p>
                      </div>
                    ) : (
                      Object.entries(tripParticipants).map(([title, data]) => (
                        <div key={title} className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-4.5 shadow-lg flex flex-col justify-between max-h-64">
                          <div className="mb-4">
                            <h4 className="text-sm font-bold text-white leading-tight">{title}</h4>
                            <span className="text-[9px] text-orange-400 uppercase font-black block mt-1">Total Joined: {data.count} Members</span>
                          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn">
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow-2xl">
                    <div className="flex justify-between items-center select-none mb-4">
                      <div>
                        <span className="text-[8px] text-gray-500 uppercase font-black tracking-widest block font-mono">Operations</span>
                        <h2 className="font-outfit text-lg font-bold uppercase text-white">Live Telemetry Control</h2>
                      </div>
                    ...
                    <div className="bg-[#070708] rounded-xl p-3.5 font-mono text-[9px] text-gray-400 space-y-1.5 h-36 overflow-y-auto border border-white/5">
                      {telemetryLogs.map((log, index) => (
                        <div key={index} className="flex gap-2">
                          <span className="text-orange-500/60 font-black">[{new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false })}]</span>
                          <span>{log}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow-2xl">
                    ...
                    <div className="space-y-3 text-xs">
                      ...
                      <div className="p-3 bg-[#070708] rounded-xl border border-white/5 flex justify-between items-center">
                        <div>
                          <div className="font-bold text-white">Munnar Team</div>
                          <div className="text-[8px] text-gray-500 mt-0.5 font-mono">Sarah Williams</div>
                        </div>
                      ...
                      <div className="p-3 bg-[#070708] rounded-xl border border-white/5 flex justify-between items-center opacity-60">
                        <div>
                          <div className="font-bold text-white">Wayanad Team</div>
                          <div className="text-[8px] text-gray-500 mt-0.5 font-mono">David Miller</div>
                        </div>
                        <span className="text-[8px] uppercase tracking-widest text-gray-500 font-bold">Offline</span>
                      </div>

                      <div className="p-3 bg-[#070708] rounded-xl border border-white/5 flex justify-between items-center opacity-60">
                        <div>
                          <div className="font-bold text-white">Nilgiri Ridge</div>
                          <div className="text-[8px] text-gray-500 mt-0.5 font-mono">HQ Comms Standby</div>
                        </div>
                        <span className="text-[8px] uppercase tracking-widest text-orange-500 font-bold">Standby</span>
                      </div>
            ...
            {activeView === 'payments' && (
              <div className="space-y-6 animate-fadeIn">
                <div>
                  <h2 className="font-outfit text-xl font-black uppercase text-white tracking-tight">Financial Operations</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 select-none">
                  ...
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow flex flex-col justify-between min-h-[120px]">
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-white font-mono mt-2">
                        ₹{Math.round((stats.totalRevenue || 123800) * 0.85).toLocaleString('en-IN')}
                      </h3>
                    </div>
                  ...
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow flex flex-col justify-between min-h-[120px]">
                    ...
                    <span className="text-[8px] text-gray-500 font-bold block mt-1.5">Bi-weekly automated ACH transfer</span>
                  </div>
                ...
                <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow-2xl select-text">
                  ...
                  {bookings.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                      {bookings.map((b) => (
                        <div key={b.ticketId} className="flex justify-between items-center py-2.5 border-b border-white/5 text-[11px] hover:bg-white/[0.01] px-2 rounded transition">
                          <div>
                            <div className="font-bold text-white">{b.fullName}</div>
                            <div className="text-[8px] text-gray-500 mt-0.5 font-mono">{b.trekTitle}</div>
                          </div>
                          <div className="text-right">
                            <div className="font-bold text-emerald-400 font-mono">+₹{b.payableAmount.toLocaleString('en-IN')}</div>
                            <div className="text-[8px] text-gray-500 font-mono mt-0.5">{b.bookingDate}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {activeView === 'analytics' && (
              <div className="space-y-6 animate-fadeIn">
                <div>
                  <h2 className="font-outfit text-xl font-black uppercase text-white tracking-tight">Analytical Telemetry</h2>
                  <p className="text-gray-400 text-xs font-light mt-0.5">Demographics, trail popularity, and hiker growth logs.</p>
                </div>
                    ...
                    <div className="space-y-3.5">
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="font-semibold text-gray-300">Munnar Mist Trek</span>
                          <span className="font-mono text-gray-500">65 Bookings</span>
                        </div>
                      ...
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="font-semibold text-gray-300">Wayanad River Camp</span>
                          <span className="font-mono text-gray-500">42 Bookings</span>
                        </div>
                        <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                      ...
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="font-semibold text-gray-300">Peak Challenge</span>
                          <span className="font-mono text-gray-500">28 Bookings</span>
                        </div>
                        <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                  ...
                  <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-5 shadow-2xl flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-5">
                        <span className="font-outfit text-4xl font-black text-white">4.9</span>
                        <div className="space-y-0.5">
                          ...
                          <p className="text-[9px] text-gray-500 font-mono">Based on 280+ verified hiker ratings</p>
                        </div>
                      </div>
                    </div>
                    <div className="border-t border-white/5 pt-3.5 mt-5 text-[9px] text-gray-400 leading-relaxed font-light">
                      98.6% of participants recommend this organizer to friends. Feedback is monitored by platform compliance teams.
                    </div>
                  </div>
            ...
            {activeView === 'notifications' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="flex justify-between items-center select-none">
                  <div>
                    <h2 className="font-outfit text-xl font-bold uppercase text-white">Notifications</h2>
                    <p className="text-gray-400 text-xs font-light mt-0.5">Stay updated with your bookings and payments.</p>
                  </div>
                  ...
                  <div className="lg:col-span-2 space-y-3">
                    {notificationsLoading && notifications.length === 0 ? (
                      <div className="space-y-4">
                        {[1, 2, 3].map(i => (
                          <div key={i} className="h-20 w-full rounded-2xl bg-white/5 border border-white/5 animate-pulse animate-fadeIn"></div>
                        ))}
                      </div>
                      ...
                      <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-12 text-center shadow-2xl flex flex-col items-center justify-center animate-fadeIn select-none">
                        <div className="h-12 w-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-xl mb-3 text-gray-500">
                          
                        </div>
                        <h4 className="font-outfit text-sm font-bold uppercase text-gray-400 mb-1">No Notifications</h4>
                        <p className="text-gray-500 text-xs max-w-xs leading-relaxed font-light">
                          {showUnreadOnly
                        ...
                        <div className="space-y-3.5">
                          ...
                          <div className="flex items-center justify-between pb-3 border-b border-white/5 select-none">
                            <div>
                              ...
                              <div className="text-[9px] text-gray-500 font-light mt-0.5">Filter read notifications.</div>
                            </div>
                            <button
                            ...
                            >
                              <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${showUnreadOnly ? 'translate-x-3.5' : 'translate-x-0'}`} />
                            </button>
                          </div>
                          ...
                          <div className="flex items-center justify-between pb-3 border-b border-white/5 select-none">
                            <div>
                              ...
                              <div className="text-[9px] text-gray-500 font-light mt-0.5">Mute dashboard indicators.</div>
                            </div>
                            <button
                            ...
                            >
                              <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${muteNotifications ? 'translate-x-3.5' : 'translate-x-0'}`} />
                            </button>
                          </div>
                          ...
                          <div className="flex items-center justify-between select-none">
                            <div>
                              ...
                              <div className="text-[9px] text-gray-500 font-light mt-0.5">Get emails for bookings.</div>
                            </div>
                            <button
                            ...
                            >
                              <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${emailAlerts ? 'translate-x-3.5' : 'translate-x-0'}`} />
                            </button>
                          </div>
                        </div>
                      </div>
